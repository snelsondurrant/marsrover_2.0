...
void loop()
{
    ...
    { // Makes sure LED is low after reconnection (otherwise 50/50)
        digitalWrite(ARDUINO_LED, LOW);
        falseLED = false;
    }
    ...
    if (currentTime - lastSlowTime >= SLOWPERIOD)
    {
        ...
        if (((digitalRead(ELEVATOR_TOP_LIMIT_SWITCH) && wheels.wheelList[6].dir == 1) || (digitalRead(ELEVATOR_BOTTOM_LIMIT_SWITCH) && wheels.wheelList[6].dir == 0)) && wheels.wheelList[6].set_speed > STOP_WHEELS)
        {
#if DEBUG
