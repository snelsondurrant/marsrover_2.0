...
public:
    GtsamLocalizerNode() : Node("gtsam_localizer_node")
    {
        ...
        global_odom_pub_ = this->create_publisher<nav_msgs::msg::Odometry>("/odometry/global", 10);

        imu_sub_ = this->create_subscription<sensor_msgs::msg::Imu>(
            "/imu", 200, std::bind(&GtsamLocalizerNode::imuCallback, this, std::placeholders::_1));
        gps_odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
            "/odometry/gps", 20, std::bind(&GtsamLocalizerNode::gpsOdomCallback, this, std::placeholders::_1));

        factor_graph_timer_ = this->create_wall_timer(
            std::chrono::milliseconds(1000), // 1 Hz factor graph update rate
            std::bind(&GtsamLocalizerNode::factorGraphTimerCallback, this));

        odom_pub_timer_ = this->create_wall_timer(
            std::chrono::milliseconds(100), // 10 Hz odometry publishing rate
...
private:
    ...
    void factorGraphTimerCallback()
    {
        ...
        if (!system_initialized_)
        {
            ...
            {
                RCLCPP_INFO(this->get_logger(), "First IMU and GPS received. Initializing system.");
    ...
    void publishTransformsAndOdometry()
    {
        ...
        {
            return; // Don't publish if TF is not available
        }
        ...
        map_to_odom_tf.transform.rotation = tf2::toMsg(map_to_odom_gtsam.rotation().toQuaternion());
        ...
        odom_msg.pose.pose = tf2::toMsg(prev_pose_);
